...
bool bishop_threat(char king_pos[3], char bishop_pos[3], char rook_pos[3]) { //returns true if the check comes from the bishop
    ...
    {
        if (abs(int(rook_pos[0]) - int(bishop_pos[0])) != abs(int(rook_pos[1]) - int(bishop_pos[1])) || abs(int(rook_pos[0]) - int(king_pos[0])) != abs(int(rook_pos[1]) - int(king_pos[1])) || int(rook_pos[1]) < int(bishop_pos[1]) && int(rook_pos[1]) < int(king_pos[1]) || int(bishop_pos[1]) > int(rook_pos[1]) && int(bishop_pos[1]) > int(king_pos[1]))
            check = true;
    }
